...
export function percentageCalculator(data, attribute, target) {
...
export function namePercentage(data, attribute2, target) {
  let total = data.length;
  let targetCount = 0;
  for (let i = 0; i < data.length; i++) {
...
export function statePercentage(data, attribute2, target) {
  // pass in male, female
  let statesObj = {};
  let statesObjF = {};
  let statesObjM = {};
...
// helper functions
export function sortProperties(obj, total) {
...
export function getDataAndLabels(dataArr) {
  let data = [];
  let labels = [];
