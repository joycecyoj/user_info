...
router.post('/info', (req, res, next) => {
  try {
    const inputData = req.body.results;
    const inputDataFormat = req.body.format;

    const genderPercentageArr = percentageCalculator(
      inputData,
      'gender',
      'female'
    );
    ...
    const statePercentageArrData = getDataAndLabels(
      statePercentage(inputData, 'state')
    ).data;
    const statePercentageArrLabel = getDataAndLabels(
      statePercentage(inputData, 'state')
    ).labels;
    const stateFemaleArrData = getDataAndLabels(
      statePercentage(inputData, 'state', 'female')
    ).data;
    const stateFemaleArrLabel = getDataAndLabels(
      statePercentage(inputData, 'state', 'female')
    ).labels;
    const stateMaleArrData = getDataAndLabels(
      statePercentage(inputData, 'state', 'male')
    ).data;
    const stateMaleArrLabel = getDataAndLabels(
      statePercentage(inputData, 'state', 'male')
    ).labels;
    ...
    let text =
      ...
      genderPercentageArr[0] +
      '/' +
      genderPercentageArr[1] +
      '\n' +
      'Percentage of first names that start with A‐M versus N‐Z: ' +
      firstNamePercentageArr[0] +
      '/' +
      firstNamePercentageArr[1] +
      '\n' +
      'Percentage of last names that start with A‐M versus N‐Z: ' +
      lastNamePercentageArr[0] +
      '/' +
      lastNamePercentageArr[1] +
      '\n' +
      'Percentage of people in each state - top 10 states: ' +
      dataAndLabel(statePercentageArrData, statePercentageArrLabel) +
      '\n' +
      'Percentage of female in each state - top 10 states: ' +
      dataAndLabel(stateFemaleArrData, stateFemaleArrLabel) +
      '\n' +
      'Percentage of male in each state - top 10 states: ' +
      dataAndLabel(stateMaleArrData, stateMaleArrLabel) +
      '\n' +
      'Percentage of people in the following age ranges: ' +
      dataAndLabel(ageArr, ageLabel) +
      '\n';
